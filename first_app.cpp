    ...
    while (!window.shouldClose()){
        ...
        for (auto it = bulletsInfo.begin(); it != bulletsInfo.end();){
            ...
            if (gameObjects.find(bulletInfo.gameObjectId) != gameObjects.end()){
				...
				if (CollisionSystem::checkCollisionOBB(mapBullet, gameObjects.at(3)).isColliding == true){
					std::printf("hited the boxed\n");
				}
            }
            it++;
        }
...
void FirstApp::loadGameObjects(){
    ...
    std::shared_ptr<MyModel> gameModel3 = MyModel::createModelFromFile(device, "models/cube.obj");
    auto cube = MyGameObject::createGameObject();
    cube.model = gameModel3;
    cube.transform.translation = {2.0f, -.5f, .0f};
