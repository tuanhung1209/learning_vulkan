...
void FirstApp::run() {
  ...
  SimpleRenderSystem simpleRenderSystem{
      device, myRenderer.getSwapChainRenderPass(),
  ...
  // std::shared_ptr<MyModel> cubeModel = MyModel::createModelFromFile(device,
  // "models/cube.obj");

  auto playerObject = MyGameObject::createGameObject();
  gameObjects.emplace(playerObject.getId(), std::move(playerObject));
  MyPlayer mainPlayer{MyModel::createModelFromFile(device, "models/cube.obj"),
                      camera, playerObject.getId()};
...
void FirstApp::loadGameObjects() {
  std::shared_ptr<MyModel> gameModel =
      MyModel::createModelFromFile(device, "models/smooth_vase.obj");
  ...
  std::shared_ptr<MyModel> gameModel1 =
      MyModel::createModelFromFile(device, "models/flat_vase.obj");
  ...
  std::shared_ptr<MyModel> gameModel2 =
      MyModel::createModelFromFile(device, "models/quad.obj");
