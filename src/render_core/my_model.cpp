...
void MyModel::createIndexBuffers(const std::vector<uint32_t> &indicies) {
    ...
    if (!haveIndexBuffer) { return; }

    VkDeviceSize bufferSize = sizeof(indicies[0]) * indexCount;
    uint32_t indexSize = sizeof(indicies[0]);

    MyBuffer statgingBuffer{myDevice, indexSize, indexCount, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT};
    ...
    indexBuffer = std::make_unique<MyBuffer>(
        myDevice, indexSize, indexCount, VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
