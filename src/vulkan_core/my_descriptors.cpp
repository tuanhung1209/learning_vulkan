...
MyDescriptorSetLayout::Builder &MyDescriptorSetLayout::Builder::addBinding(uint32_t binding,
...
MyDescriptorSetLayout::MyDescriptorSetLayout(
    ...
    if (vkCreateDescriptorSetLayout(myDevice.device(), &descriptorSetLayoutInfo, nullptr,
...
MyDescriptorPool::Builder &MyDescriptorPool::Builder::addPoolSize(VkDescriptorType descriptorType,
                                                                  uint32_t count) {
    poolSizes.push_back({descriptorType, count});
    return *this;
}

MyDescriptorPool::Builder &MyDescriptorPool::Builder::setPoolFlags(VkDescriptorPoolCreateFlags flags) {
    poolFlags = flags;
    return *this;
}
...
MyDescriptorPool::MyDescriptorPool(Device &lveDevice, uint32_t maxSets, VkDescriptorPoolCreateFlags poolFlags,
...
MyDescriptorPool::~MyDescriptorPool() { vkDestroyDescriptorPool(myDevice.device(), descriptorPool, nullptr); }

bool MyDescriptorPool::allocateDescriptor(const VkDescriptorSetLayout descriptorSetLayout,
    ...
    if (vkAllocateDescriptorSets(myDevice.device(), &allocInfo, &descriptor) != VK_SUCCESS) { return false; }
    return true;
}

void MyDescriptorPool::freeDescriptors(std::vector<VkDescriptorSet> &descriptors) const {
    vkFreeDescriptorSets(myDevice.device(), descriptorPool, static_cast<uint32_t>(descriptors.size()),
                         descriptors.data());
}

void MyDescriptorPool::resetPool() { vkResetDescriptorPool(myDevice.device(), descriptorPool, 0); }
...
MyDescriptorWriter &MyDescriptorWriter::writeBuffer(uint32_t binding, VkDescriptorBufferInfo *bufferInfo) {
...
MyDescriptorWriter &MyDescriptorWriter::writeImage(uint32_t binding, VkDescriptorImageInfo *imageInfo) {
    assert(setLayout.bindings.count(binding) == 1 && "Layout does not contain specified binding");
